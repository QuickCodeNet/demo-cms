...
using Moq;
using QuickCode.DemoCms.SiteManagementModule.Persistence.Repositories;
...
namespace QuickCode.DemoCms.SiteManagementModule.Persistence.Tests.Repositories
{
    public class SiteLanguageRepositoryTests : IDisposable
    {
        ...
        private Mock<ILogger<SiteLanguageRepository>> loggerMock = null;
        private readonly ITestOutputHelper _output;
        public SiteLanguageRepositoryTests(ITestOutputHelper output)
        {
            _output = output;
            CreateDatabase();
        }

        private SiteLanguageRepository CreateRepository()
        {
            var writeContext = new WriteDbContext(writeOptions);
            var readContext = new ReadDbContext(readOptions);
            return new SiteLanguageRepository(loggerMock.Object, writeContext, readContext);
        }
        ...
        private void CreateDatabase()
        {
            ...
            loggerMock = new Mock<ILogger<SiteLanguageRepository>>();
        }
        ...
        public async Task InsertAsync_Should_Insert_Item_Successfully()
        {
            ...
            var itemDto = TestDataGenerator.CreateFake<SiteLanguageDto>("tr");
            // Act
        ...
        public async Task InsertAsync_Should_Generate_Unique_Ids()
        {
            // Arrange
            var repository = CreateRepository();
            var fakeItem1 = TestDataGenerator.CreateFake<SiteLanguageDto>();
            var fakeItem2 = TestDataGenerator.CreateFake<SiteLanguageDto>("tr");
            // Act
            ...
            Assert.NotEqual(result1.Value.SiteId, result2.Value.SiteId);
        }
        ...
        public async Task GetByPkAsync_Should_Return_Item_When_Exists()
        {
            // Arrange
            var repository = CreateRepository();
            var itemDto = TestDataGenerator.CreateFake<SiteLanguageDto>();
            var insertedItem = await repository.InsertAsync(itemDto);
            Assert.Equal(ResultCodeSuccess, insertedItem.Code);
            // Act
            var result = await repository.GetByPkAsync(insertedItem.Value.SiteId, insertedItem.Value.LanguageId);
            // Assert
            Assert.Equal(ResultCodeSuccess, result.Code);
            itemDto.AssertPropertiesEqual(result.Value);
        }
        ...
        public async Task GetByPkAsync_Should_Return_NotFound_When_Item_Not_Exists()
        {
            // Arrange
            var repository = CreateRepository();
            var notExistsItem = TestDataGenerator.CreateFake<SiteLanguageDto>("tr");
            // Act
            var result = await repository.GetByPkAsync(notExistsItem.SiteId, notExistsItem.LanguageId);
            // Assert
            Assert.Equal(ResultCodeNotFound, result.Code);
            Assert.Equal("Not found in SiteLanguage", result.Message);
        }
        ...
        public async Task UpdateAsync_Should_Update_Item_Successfully()
        {
            ...
            var updates = insertedItem.GenerateRandomUpdates(["SiteId", "LanguageId"]);
            ...
            var updatedItem = await repository.GetByPkAsync(insertedItem.SiteId, insertedItem.LanguageId);
        ...
        public async Task DeleteAsync_Should_Delete_Item_Successfully()
        {
            ...
            var deletedItem = await repository.GetByPkAsync(insertedItem.Value.SiteId, insertedItem.Value.LanguageId);
            Assert.Equal(ResultCodeNotFound, deletedItem.Code);
        }

        [Fact]
        public async Task ListAsync_Should_Return_All_SiteLanguage()
        {
            // Arrange
            CreateDatabase();
            var repository = CreateRepository();
            var fakeItems = TestDataGenerator.CreateFakes<SiteLanguageDto>("tr");
        ...
        public async Task ListAsync_With_Pagination_Should_Return_Correct_Page()
        {
            // Arrange
            var repository = CreateRepository();
            var fakeItems = TestDataGenerator.CreateFakes<SiteLanguageDto>("en", 5);
            foreach (var fakeItem in fakeItems)
            {
                var insertResponse = await repository.InsertAsync(fakeItem);
                if (insertResponse.Code != ResultCodeSuccess)
