...
{
    public class InsertSiteLanguageCommandTests : IDisposable
    {
        ...
        public InsertSiteLanguageCommandTests()
        {
        ...
        public async Task InsertAsync_Should_Return_Success_When_Valid_Request()
        {
            ...
            var fakeResponse = new RepoResponse<SiteLanguageDto>(fakeDto, "Success");
            _repositoryMock.Setup(r => r.InsertAsync(It.IsAny<SiteLanguageDto>())).ReturnsAsync(fakeResponse);
            // Act
            var result = await _service.InsertAsync(fakeDto);
            // Assert
            Assert.Equal(ResultCodeSuccess, result.Code);
            Assert.Equal(fakeDto, result.Value);
            _repositoryMock.Verify(r => r.InsertAsync(It.IsAny<SiteLanguageDto>()), Times.Once);
        }
        ...
        public async Task InsertAsync_Should_Return_NotFound_When_Repository_Returns_404()
        {
            // Arrange
            var fakeDto = TestDataGenerator.CreateFake<SiteLanguageDto>("tr");
            var fakeResponse = new RepoResponse<SiteLanguageDto>
            {
