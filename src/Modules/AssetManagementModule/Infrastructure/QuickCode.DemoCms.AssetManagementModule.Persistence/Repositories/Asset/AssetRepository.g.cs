...
namespace QuickCode.DemoCms.AssetManagementModule.Persistence.Repositories
{
    public partial class AssetRepository : BaseRepository, IAssetRepository
    {
        ...
        public AssetRepository(ILogger<AssetRepository> logger, WriteDbContext writeContext, ReadDbContext readContext) : base(logger, "Asset")
        {
            _writeContext = writeContext;
            _readContext = readContext;
        }

        public async Task<RepoResponse<AssetDto>> InsertAsync(AssetDto value)
        {
            return await ExecuteWithExceptionHandling("Insert", async () =>
            {
                var entity = value.ToEntity();
                await _writeContext.Asset.AddAsync(entity);
                await _writeContext.SaveChangesAsync();
                var resultDto = entity.ToDto();
                return new RepoResponse<AssetDto>(resultDto, "Success");
            });
        }

        public async Task<RepoResponse<bool>> UpdateAsync(AssetDto value)
        {
            return await ExecuteWithExceptionHandling("Update", async () =>
            {
                _writeContext.Set<Domain.Entities.Asset>().Update(value.ToEntity());
                await _writeContext.SaveChangesAsync();
                return new RepoResponse<bool>(true, "Success");
            });
        }

        public async Task<RepoResponse<bool>> DeleteAsync(AssetDto value)
        {
            return await ExecuteWithExceptionHandling("Delete", async () =>
            {
                _writeContext.Asset.Remove(value.ToEntity());
                await _writeContext.SaveChangesAsync();
                return new RepoResponse<bool>(true, "Success");
            });
        }

        public async Task<RepoResponse<AssetDto>> GetByPkAsync(int id)
        {
            return await ExecuteWithExceptionHandling("GetByPk", async () =>
            {
                var result =
                    from A in _readContext.Asset
                    where A.Id.Equals(id)
                    select A;
                var response = await result.FirstOrDefaultAsync();
                return response == null ? new RepoResponse<AssetDto>
                {
                    Code = 404,
                    Message = "Not found in Asset"
                }

                : new RepoResponse<AssetDto>(response.ToDto(), "Success");
            });
        }

        public async Task<RepoResponse<List<AssetDto>>> ListAsync(int? pageNumber = null, int? pageSize = null)
        {
            return await ExecuteWithExceptionHandling("Select", async () =>
            {
                ...
                {
                    return new RepoResponse<List<AssetDto>>
                    {
                        Code = 404,
                        Message = $"Page Number must be greater than {ConfigurationConstants.MinPageNumber}"};
                }

                var query = _readContext.Asset.OrderBy(o => o.Id).AsQueryable();
                ...
                return new RepoResponse<List<AssetDto>>(result.ToDto(), "Success");
            });
        }
        ...
        public async Task<RepoResponse<int>> CountAsync()
        {
            ...
            {
                return new RepoResponse<int>(await _readContext.Asset.CountAsync(), "Success");
            });
        }
    }
}