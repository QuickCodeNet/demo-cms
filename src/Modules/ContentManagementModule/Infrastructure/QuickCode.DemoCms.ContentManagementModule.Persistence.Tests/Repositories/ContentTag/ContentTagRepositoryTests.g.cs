...
using Moq;
using QuickCode.DemoCms.ContentManagementModule.Persistence.Repositories;
...
namespace QuickCode.DemoCms.ContentManagementModule.Persistence.Tests.Repositories
{
    public class ContentTagRepositoryTests : IDisposable
    {
        ...
        private Mock<ILogger<ContentTagRepository>> loggerMock = null;
        private readonly ITestOutputHelper _output;
        public ContentTagRepositoryTests(ITestOutputHelper output)
        {
            _output = output;
            CreateDatabase();
        }

        private ContentTagRepository CreateRepository()
        {
            var writeContext = new WriteDbContext(writeOptions);
            var readContext = new ReadDbContext(readOptions);
            return new ContentTagRepository(loggerMock.Object, writeContext, readContext);
        }
        ...
        private void CreateDatabase()
        {
            ...
            loggerMock = new Mock<ILogger<ContentTagRepository>>();
        }
        ...
        public async Task InsertAsync_Should_Insert_Item_Successfully()
        {
            ...
            var itemDto = TestDataGenerator.CreateFake<ContentTagDto>("tr");
            // Act
        ...
        public async Task InsertAsync_Should_Generate_Unique_Ids()
        {
            // Arrange
            var repository = CreateRepository();
            var fakeItem1 = TestDataGenerator.CreateFake<ContentTagDto>();
            var fakeItem2 = TestDataGenerator.CreateFake<ContentTagDto>("tr");
            // Act
            ...
            Assert.NotEqual(result1.Value.ContentTypeKind, result2.Value.ContentTypeKind);
        }
        ...
        public async Task GetByPkAsync_Should_Return_Item_When_Exists()
        {
            // Arrange
            var repository = CreateRepository();
            var itemDto = TestDataGenerator.CreateFake<ContentTagDto>();
            var insertedItem = await repository.InsertAsync(itemDto);
            Assert.Equal(ResultCodeSuccess, insertedItem.Code);
            // Act
            var result = await repository.GetByPkAsync(insertedItem.Value.ContentTypeKind, insertedItem.Value.ContentId, insertedItem.Value.TagId);
            // Assert
            Assert.Equal(ResultCodeSuccess, result.Code);
            itemDto.AssertPropertiesEqual(result.Value);
        }
        ...
        public async Task GetByPkAsync_Should_Return_NotFound_When_Item_Not_Exists()
        {
            // Arrange
            var repository = CreateRepository();
            var notExistsItem = TestDataGenerator.CreateFake<ContentTagDto>("tr");
            // Act
            var result = await repository.GetByPkAsync(notExistsItem.ContentTypeKind, notExistsItem.ContentId, notExistsItem.TagId);
            // Assert
            Assert.Equal(ResultCodeNotFound, result.Code);
            Assert.Equal("Not found in ContentTag", result.Message);
        }
        ...
        public async Task UpdateAsync_Should_Update_Item_Successfully()
        {
            ...
            var updates = insertedItem.GenerateRandomUpdates(["ContentTypeKind", "ContentId", "TagId"]);
            ...
            var updatedItem = await repository.GetByPkAsync(insertedItem.ContentTypeKind, insertedItem.ContentId, insertedItem.TagId);
        ...
        public async Task DeleteAsync_Should_Delete_Item_Successfully()
        {
            ...
            var deletedItem = await repository.GetByPkAsync(insertedItem.Value.ContentTypeKind, insertedItem.Value.ContentId, insertedItem.Value.TagId);
            Assert.Equal(ResultCodeNotFound, deletedItem.Code);
        }

        [Fact]
        public async Task ListAsync_Should_Return_All_ContentTag()
        {
            // Arrange
            CreateDatabase();
            var repository = CreateRepository();
            var fakeItems = TestDataGenerator.CreateFakes<ContentTagDto>("tr");
        ...
        public async Task ListAsync_With_Pagination_Should_Return_Correct_Page()
        {
            // Arrange
            var repository = CreateRepository();
            var fakeItems = TestDataGenerator.CreateFakes<ContentTagDto>("en", 5);
            foreach (var fakeItem in fakeItems)
            {
                var insertResponse = await repository.InsertAsync(fakeItem);
                if (insertResponse.Code != ResultCodeSuccess)
