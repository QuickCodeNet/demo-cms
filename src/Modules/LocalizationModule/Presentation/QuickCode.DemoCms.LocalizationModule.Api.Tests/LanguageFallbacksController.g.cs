...
using QuickCode.DemoCms.LocalizationModule.Api.Controllers;
...
{
    public class LanguageFallbacksControllerTests
    {
        private readonly Mock<ILanguageFallbackService> _serviceMock = new();
        private readonly Mock<ILogger<LanguageFallbacksController>> _loggerMock = new();
        private readonly LanguageFallbacksController _controller;
        public LanguageFallbacksControllerTests()
        {
            _controller = new LanguageFallbacksController(_serviceMock.Object, null, _loggerMock.Object);
        }
        ...
        {
            // Arrange
            var fakeList = TestDataGenerator.CreateFakes<LanguageFallbackDto>("tr");
            _serviceMock.Setup(s => s.ListAsync(It.IsAny<int?>(), It.IsAny<int?>())).ReturnsAsync(new Response<List<LanguageFallbackDto>> { Code = 0, Value = fakeList });
            // Act
        ...
        {
            // Arrange
            _serviceMock.Setup(s => s.ListAsync(It.IsAny<int?>(), It.IsAny<int?>())).ReturnsAsync(new Response<List<LanguageFallbackDto>> { Code = 1, Message = "Error" });
            // Act
            var result = await _controller.ListAsync(1, 10);
            // Assert
        ...
        public async Task GetItemAsync_Should_Return_Ok_When_Found()
        {
            ...
            _serviceMock.Setup(s => s.GetItemAsync(It.IsAny<int>(), It.IsAny<int>())).ReturnsAsync(new Response<LanguageFallbackDto> { Code = 0, Value = fakeDto });
            // Act
            var result = await _controller.GetItemAsync(fakeDto.LanguageId, fakeDto.FallbackLanguageId);
            // Assert
            var okResult = Assert.IsType<OkObjectResult>(result);
        ...
        public async Task GetItemAsync_Should_Return_NotFound_When_404()
        {
            // Arrange
            var fakeDto = TestDataGenerator.CreateFake<LanguageFallbackDto>("tr");
            _serviceMock.Setup(s => s.GetItemAsync(It.IsAny<int>(), It.IsAny<int>())).ReturnsAsync(new Response<LanguageFallbackDto> { Code = 404, Message = "Not found" });
            // Act
            var result = await _controller.GetItemAsync(fakeDto.LanguageId, fakeDto.FallbackLanguageId);
            // Assert
            Assert.IsType<NotFoundObjectResult>(result);
        ...
        public async Task GetItemAsync_Should_Return_BadRequest_When_Fail()
        {
            // Arrange
            var fakeDto = TestDataGenerator.CreateFake<LanguageFallbackDto>("tr");
            _serviceMock.Setup(s => s.GetItemAsync(It.IsAny<int>(), It.IsAny<int>())).ReturnsAsync(new Response<LanguageFallbackDto> { Code = 1, Message = "Error" });
            // Act
            var result = await _controller.GetItemAsync(fakeDto.LanguageId, fakeDto.FallbackLanguageId);
            // Assert
        ...
        public async Task InsertAsync_Should_Return_Created_When_Success()
        {
            // Arrange
            var fakeDto = TestDataGenerator.CreateFake<LanguageFallbackDto>("tr");
            _serviceMock.Setup(s => s.InsertAsync(It.IsAny<LanguageFallbackDto>())).ReturnsAsync(new Response<LanguageFallbackDto> { Code = 0, Value = fakeDto });
            // Act
        ...
        public async Task InsertAsync_Should_Return_BadRequest_When_Fail()
        {
            // Arrange
            var fakeDto = TestDataGenerator.CreateFake<LanguageFallbackDto>("tr");
            _serviceMock.Setup(s => s.InsertAsync(It.IsAny<LanguageFallbackDto>())).ReturnsAsync(new Response<LanguageFallbackDto> { Code = 1, Message = "Insert failed" });
            // Act
            var result = await _controller.InsertAsync(fakeDto);
            // Assert
            Assert.IsType<BadRequestObjectResult>(result);
        ...
        public async Task UpdateAsync_Should_Return_Ok_When_Success()
        {
            // Arrange
            var fakeDto = TestDataGenerator.CreateFake<LanguageFallbackDto>("tr");
            _serviceMock.Setup(s => s.UpdateAsync(It.IsAny<int>(), It.IsAny<int>(), It.IsAny<LanguageFallbackDto>())).ReturnsAsync(new Response<bool> { Code = 0, Value = true });
            // Act
            var result = await _controller.UpdateAsync(fakeDto.LanguageId, fakeDto.FallbackLanguageId, fakeDto);
            // Assert
            var okResult = Assert.IsType<OkObjectResult>(result);
        ...
        public async Task UpdateAsync_Should_Return_NotFound_When_404()
        {
            // Arrange
            var fakeDto = TestDataGenerator.CreateFake<LanguageFallbackDto>("tr");
            _serviceMock.Setup(s => s.UpdateAsync(It.IsAny<int>(), It.IsAny<int>(), It.IsAny<LanguageFallbackDto>())).ReturnsAsync(new Response<bool> { Code = 404, Message = "Not found" });
            // Act
            var result = await _controller.UpdateAsync(fakeDto.LanguageId, fakeDto.FallbackLanguageId, fakeDto);
            // Assert
            Assert.IsType<NotFoundObjectResult>(result);
        ...
        public async Task UpdateAsync_Should_Return_BadRequest_When_Fail()
        {
            // Arrange
            var fakeDto = TestDataGenerator.CreateFake<LanguageFallbackDto>("tr");
            _serviceMock.Setup(s => s.UpdateAsync(It.IsAny<int>(), It.IsAny<int>(), It.IsAny<LanguageFallbackDto>())).ReturnsAsync(new Response<bool> { Code = 1, Message = "Update failed" });
            // Act
            var result = await _controller.UpdateAsync(fakeDto.LanguageId, fakeDto.FallbackLanguageId, fakeDto);
            // Assert
            Assert.IsType<BadRequestObjectResult>(result);
        ...
        public async Task DeleteAsync_Should_Return_Ok_When_Success()
        {
            ...
            // Act
            var result = await _controller.DeleteAsync(fakeDto.LanguageId, fakeDto.FallbackLanguageId);
            // Assert
            var okResult = Assert.IsType<OkObjectResult>(result);
            Assert.True((bool)okResult.Value);
        }
