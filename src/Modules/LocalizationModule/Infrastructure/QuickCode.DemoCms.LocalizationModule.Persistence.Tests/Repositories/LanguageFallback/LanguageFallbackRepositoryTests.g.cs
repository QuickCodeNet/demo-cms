...
using Moq;
using QuickCode.DemoCms.LocalizationModule.Persistence.Repositories;
...
namespace QuickCode.DemoCms.LocalizationModule.Persistence.Tests.Repositories
{
    public class LanguageFallbackRepositoryTests : IDisposable
    {
        ...
        private Mock<ILogger<LanguageFallbackRepository>> loggerMock = null;
        private readonly ITestOutputHelper _output;
        public LanguageFallbackRepositoryTests(ITestOutputHelper output)
        {
            _output = output;
            CreateDatabase();
        }

        private LanguageFallbackRepository CreateRepository()
        {
            var writeContext = new WriteDbContext(writeOptions);
            var readContext = new ReadDbContext(readOptions);
            return new LanguageFallbackRepository(loggerMock.Object, writeContext, readContext);
        }
        ...
        private void CreateDatabase()
        {
            ...
            loggerMock = new Mock<ILogger<LanguageFallbackRepository>>();
        }
        ...
        public async Task InsertAsync_Should_Insert_Item_Successfully()
        {
            ...
            var itemDto = TestDataGenerator.CreateFake<LanguageFallbackDto>("tr");
            // Act
        ...
        public async Task InsertAsync_Should_Generate_Unique_Ids()
        {
            // Arrange
            var repository = CreateRepository();
            var fakeItem1 = TestDataGenerator.CreateFake<LanguageFallbackDto>();
            var fakeItem2 = TestDataGenerator.CreateFake<LanguageFallbackDto>("tr");
            // Act
            ...
            Assert.NotEqual(result1.Value.LanguageId, result2.Value.LanguageId);
        }
        ...
        public async Task GetByPkAsync_Should_Return_Item_When_Exists()
        {
            // Arrange
            var repository = CreateRepository();
            var itemDto = TestDataGenerator.CreateFake<LanguageFallbackDto>();
            var insertedItem = await repository.InsertAsync(itemDto);
            Assert.Equal(ResultCodeSuccess, insertedItem.Code);
            // Act
            var result = await repository.GetByPkAsync(insertedItem.Value.LanguageId, insertedItem.Value.FallbackLanguageId);
            // Assert
            Assert.Equal(ResultCodeSuccess, result.Code);
            itemDto.AssertPropertiesEqual(result.Value);
        }
        ...
        public async Task GetByPkAsync_Should_Return_NotFound_When_Item_Not_Exists()
        {
            // Arrange
            var repository = CreateRepository();
            var notExistsItem = TestDataGenerator.CreateFake<LanguageFallbackDto>("tr");
            // Act
            var result = await repository.GetByPkAsync(notExistsItem.LanguageId, notExistsItem.FallbackLanguageId);
            // Assert
            Assert.Equal(ResultCodeNotFound, result.Code);
            Assert.Equal("Not found in LanguageFallback", result.Message);
        }
        ...
        public async Task UpdateAsync_Should_Update_Item_Successfully()
        {
            ...
            var updates = insertedItem.GenerateRandomUpdates(["LanguageId", "FallbackLanguageId"]);
            ...
            var updatedItem = await repository.GetByPkAsync(insertedItem.LanguageId, insertedItem.FallbackLanguageId);
        ...
        public async Task DeleteAsync_Should_Delete_Item_Successfully()
        {
            ...
            var deletedItem = await repository.GetByPkAsync(insertedItem.Value.LanguageId, insertedItem.Value.FallbackLanguageId);
            Assert.Equal(ResultCodeNotFound, deletedItem.Code);
        }

        [Fact]
        public async Task ListAsync_Should_Return_All_LanguageFallback()
        {
            // Arrange
            CreateDatabase();
            var repository = CreateRepository();
            var fakeItems = TestDataGenerator.CreateFakes<LanguageFallbackDto>("tr");
        ...
        public async Task ListAsync_With_Pagination_Should_Return_Correct_Page()
        {
            // Arrange
            var repository = CreateRepository();
            var fakeItems = TestDataGenerator.CreateFakes<LanguageFallbackDto>("en", 5);
            foreach (var fakeItem in fakeItems)
            {
                var insertResponse = await repository.InsertAsync(fakeItem);
                if (insertResponse.Code != ResultCodeSuccess)
